...
import Graphics.Gloss (Color, Picture (..), Point, blue, cyan, green, orange, red, translate, yellow)
import World
...
paintTile thisGame thisTile =
  let block = blockImgs thisGame !! findIndex (tileColor thisTile)
   in if tileInBounds thisTile
...
shiftTile ShiftDown thisTile =
  thisTile
    { tileLocale =
        (id *** flip (-) 1) $
          tileLocale thisTile
    }
shiftTile ShiftLeft thisTile =
  thisTile
    { tileLocale =
        (flip (-) 1 *** id) $
          tileLocale thisTile
    }
shiftTile ShiftRight thisTile =
  thisTile
    { tileLocale =
        ((+) 1 *** id) $
          tileLocale thisTile
...
gravitateTile thisTile thisGame acc =
  gravitateTile
    thisTile
      { tileLocale =
          (id *** flip (-) 1) $
